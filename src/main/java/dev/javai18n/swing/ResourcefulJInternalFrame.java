...
/**
 * A JInternalFrame that supports localizing the name, tool tip, font, title, frame icon, accessible name
 * and accessible description.
 */
public class ResourcefulJInternalFrame extends JInternalFrame implements Resourceful, LocaleEventListener
{
     ...
     * @param resource A Resource containing a JInternalFramePropertyBundle that holds the localized values for the
     *                 JInternalFrame.
     * @return A ResourcefulJInternalFrame with updated Locale-specific values that is registered to listen to
     *         LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulJInternalFrame create(Resource resource)
    {
        ResourcefulJInternalFrame frame = new ResourcefulJInternalFrame(resource);
        frame.initialize();
        return frame;
    }
     ...
     *                 JInternalFrame.
     */
    protected ResourcefulJInternalFrame(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            JInternalFramePropertyBundle props = (JInternalFramePropertyBundle) getResource().getObject();
            ...
            Icon frameIcon = props.getFrameIcon();
            if (null != frameIcon) setFrameIcon(frameIcon);
        }
        catch (MissingResourceException ex)
        {
            SWING_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.internal.frame", ex);
        }
    }
     ...
     * @return the Resource holding Locale specific values for the JInternalFrame.
     */
    @Override
     ...
     * @param resource the Resource holding Locale specific values for the JInternalFrame.
     */
    @Override
