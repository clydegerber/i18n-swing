...
/**
 * A JPasswordField that supports localizing the name, tool tip, font, accessible name and accessible
 * description, and participates in locale change events so that assistive technology always receives
 * correct locale-specific metadata.
 */
public class ResourcefulJPasswordField extends JPasswordField implements Resourceful, LocaleEventListener
{
     ...
     * @return A ResourcefulJPasswordField registered to listen to LocaleEvents from the resource's source.
     */
    public static ResourcefulJPasswordField create(Resource resource)
    {
        ResourcefulJPasswordField field = new ResourcefulJPasswordField(resource);
        field.initialize();
        return field;
    }
     ...
     */
    protected ResourcefulJPasswordField(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            SWING_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.password.field", ex);
        }
    }
