...
/**
 * A {@link JSpinner.DefaultEditor} that participates in locale change events, updating its
 * accessible metadata, font, name, and tooltip whenever the application locale changes.
 *
 * <p>{@code JSpinner.DefaultEditor} is the base editor panel embedded inside a
 * {@link javax.swing.JSpinner}.  It contains a formatted text field that displays the current
 * spinner value.  The locale-specific format of that field is the caller's responsibility (see
 * {@link ResourcefulJSpinnerDateEditor} and {@link ResourcefulJSpinnerNumberEditor} for typed
 * subclasses that carry format strings).  This class handles the accessible metadata that
 * assistive technologies use to describe the editor.</p>
 */
public class ResourcefulJSpinnerDefaultEditor extends JSpinner.DefaultEditor
        implements Resourceful, LocaleEventListener
{
     ...
     * @return A ResourcefulJSpinnerDefaultEditor registered to listen to LocaleEvents from the resource's source.
     */
    public static ResourcefulJSpinnerDefaultEditor create(Resource resource, JSpinner spinner)
    {
        ResourcefulJSpinnerDefaultEditor editor =
                new ResourcefulJSpinnerDefaultEditor(resource, spinner);
        editor.initialize();
        return editor;
    }
     ...
     * @param spinner  The {@link JSpinner} this editor is associated with.
     */
    protected ResourcefulJSpinnerDefaultEditor(Resource resource, JSpinner spinner)
    {
        super(spinner);
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        catch (MissingResourceException ex)
        {
            SWING_LOGGER.log(System.Logger.Level.DEBUG,
                    "missing.resource.for.spinner.default.editor", ex);
        }
    }
