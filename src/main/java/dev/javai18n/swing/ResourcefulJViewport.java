...
/**
 * A JViewport that participates in locale change events, updating its accessible metadata,
 * font, name, and tooltip whenever the application locale changes.
 *
 * <p>{@code JViewport} is the scrollable-view window used inside {@link javax.swing.JScrollPane}.
 * Although it is normally created and managed by {@code JScrollPane}, it can be directly
 * instantiated when building custom scrollable layouts.  A {@code ResourcefulJViewport} ensures
 * its accessible properties stay consistent with the locale-aware UI.</p>
 */
public class ResourcefulJViewport extends JViewport implements Resourceful, LocaleEventListener
{
     ...
     * @return A ResourcefulJViewport registered to listen to LocaleEvents from the resource's source.
     */
    public static ResourcefulJViewport create(Resource resource)
    {
        ResourcefulJViewport viewport = new ResourcefulJViewport(resource);
        viewport.initialize();
        return viewport;
    }
    ...
    protected ResourcefulJViewport(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            SWING_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.viewport", ex);
        }
    }
