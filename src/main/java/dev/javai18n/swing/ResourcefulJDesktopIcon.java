...
/**
 * A {@link JInternalFrame.JDesktopIcon} that participates in locale change events, updating its
 * accessible metadata, font, name, tooltip, and repainting its icon whenever the application
 * locale changes.
 *
 * <p>{@code JInternalFrame.JDesktopIcon} is the iconified representation of a
 * {@link JInternalFrame} shown in a {@link javax.swing.JDesktopPane} when the frame is minimised.
 * Its accessible description is important for users of assistive technologies who navigate an MDI
 * desktop.  A {@code ResourcefulJDesktopIcon} keeps those descriptions locale-consistent.</p>
 */
public class ResourcefulJDesktopIcon extends JInternalFrame.JDesktopIcon
        implements Resourceful, LocaleEventListener
{
     ...
     * @return A ResourcefulJDesktopIcon registered to listen to LocaleEvents from the resource's source.
     */
    public static ResourcefulJDesktopIcon create(Resource resource, JInternalFrame internalFrame)
    {
        ResourcefulJDesktopIcon icon = new ResourcefulJDesktopIcon(resource, internalFrame);
        icon.initialize();
        return icon;
    }
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            SWING_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.desktop.icon", ex);
        }
        repaint();
    }
