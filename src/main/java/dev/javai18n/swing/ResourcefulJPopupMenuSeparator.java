...
/**
 * A {@link JPopupMenu.Separator} that participates in locale change events, updating its accessible
 * metadata, font, name, and tooltip whenever the application locale changes.
 *
 * <p>{@code JPopupMenu.Separator} is a specialised {@link javax.swing.JSeparator} used inside
 * {@link javax.swing.JPopupMenu} instances.  While it is primarily a visual divider, its
 * accessible properties can carry locale-sensitive descriptions for assistive technologies.</p>
 */
public class ResourcefulJPopupMenuSeparator extends JPopupMenu.Separator
        implements Resourceful, LocaleEventListener
{
     ...
     * @return A ResourcefulJPopupMenuSeparator registered to listen to LocaleEvents from the resource's source.
     */
    public static ResourcefulJPopupMenuSeparator create(Resource resource)
    {
        ResourcefulJPopupMenuSeparator sep = new ResourcefulJPopupMenuSeparator(resource);
        sep.initialize();
        return sep;
    }
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        catch (MissingResourceException ex)
        {
            SWING_LOGGER.log(System.Logger.Level.DEBUG,
                    "missing.resource.for.popup.menu.separator", ex);
        }
    }
