...
/**
 * A JProgressBar that supports localizing the name, tool tip, font, progress display text, accessible name
 * and accessible description.
 */
public class ResourcefulJProgressBar extends JProgressBar implements Resourceful, LocaleEventListener
{
     ...
     * @param resource A Resource containing a JProgressBarPropertyBundle that holds the localized values for the
     *                 JProgressBar.
     * @return A ResourcefulJProgressBar with updated Locale-specific values that is registered to listen to
     *         LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulJProgressBar create(Resource resource)
    {
        ResourcefulJProgressBar progressBar = new ResourcefulJProgressBar(resource);
     ...
     *                 JProgressBar.
     */
    protected ResourcefulJProgressBar(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            JProgressBarPropertyBundle props = (JProgressBarPropertyBundle) getResource().getObject();
            ...
            String progressString = props.getProgressString();
            if (null != progressString)
            {
        ...
        {
            SWING_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.progress.bar", ex);
        }
    }
     ...
     * @return the Resource holding Locale specific values for the JProgressBar.
     */
    @Override
     ...
     * @param resource the Resource holding Locale specific values for the JProgressBar.
     */
    @Override
